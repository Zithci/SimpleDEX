    ...
    const calculateQuote = (inputAmount, inputReserve, outputReserve) => {
      const  amountIn = BigInt(inputAmount);
      const reserveIn = BigInt(inputReserve);
      const reserveOut  = BigInt(outputReserve);

      if(reserveIn === 0n) return 0n; //preventing dividing by zero

      // formula : (input * stocklawan) / stockinput 
    ...
    export default function LiquidityInterface({ signer,onTxSuccess }) {
      ...
      const handleAmountAChange = async (val) => {
        ...
        // get data from the storage(gudang/reserve)
        const dex = new Contract(CONTRACTS.SimpleDEX, ABIS.SimpleDEX, signer);
        const [resA, resB ] = await dex.getReserves();

        // if the storage is empty, return
        if(resA > 0n || resB > 0n) {
          const valWei =  parseEther(val);
          const quoteB =  calculateQuote(valWei, resA, resB);
          setAmountB(formatEther(quoteB)); //auto update B
      ...
      //#B
        const handleAmountBChange = async (val) => {
        setAmountB(val);
        ...
        if(resA > 0n || resB > 0n) {
          const valWei =  parseEther(val);
          const quoteA =  calculateQuote(valWei, resB, resA);
          setAmountB(formatEther(quoteA)); //auto update B
        }
        }catch (err){
          console.error('Error calculating quote for Token A:', err);
        }
      };
